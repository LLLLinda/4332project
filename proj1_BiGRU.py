...
if __name__ == '__main__':
    ...
    output_size = K
    print("Input size: %f ; Output Size: %f" % (input_size, output_size))
    # New model
    ...
    # GRU layer
    gru = keras.layers.Bidirectional(GRU(units=hidden_size))(e_d)

    # dense layer
    d = Dense(hidden_size, activation='relu', kernel_regularizer=regularizers.l2(0.01))(gru)

    # output layer
    y = Dense(K, activation='softmax')(d)
    ...
    optimizer = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0, amsgrad=True)
    ...
    # training
    history = model.fit(train_data_matrix, train_data_label, epochs=total_epoch, batch_size=batch_size, callbacks=[es, mc])
    ...
    # predicting
    # test_pre = saved_model.predict(test_data_matrix, batch_size=batch_size).argmax(axis=-1) + 1
    # sub_df = pd.DataFrame()
    # sub_df["review_id"] = test_id_list
    # sub_df["pre"] = test_pre
    # sub_df.to_csv("pre_t1.csv", index=False)
